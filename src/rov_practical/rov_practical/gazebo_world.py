...
class GazeboWorld(Node):
    ...
    def GetSelfOdomeInfo(self):
        euler = tf.euler_from_quaternion(self.quaternion)
        Eular = euler[2]
    ...
    def SetObjectPose(self, name='mobile_base', random_flag=False):
        if name == 'mobile_base':
            ...
            if rand < 0.2:
                ...
                quaternion = tf.quaternion_from_euler(0., 0., np.pi / 2 + np.random.uniform(-np.pi / 6, np.pi / 6))
                ...
                quaternion = tf.quaternion_from_euler(0., 0., np.random.uniform(-np.pi / 4, 3 * np.pi / 4))
                ...
                quaternion = tf.quaternion_from_euler(0., 0., random_angle)
                if random.random() < 0.5:
                    self.goal = (-1.5 + np.random.uniform(-0.5, 0.5), 6.5 + np.random.uniform(-0.5, 0.5))
                else:
                    self.goal = (1.5 + np.random.uniform(-0.5, 0.5), 6.5 + np.random.uniform(-0.5, 0.5))
                ...
                quaternion = tf.quaternion_from_euler(0., 0., np.random.uniform(-5 * np.pi / 6, -np.pi / 6))
                if random.random() < 0.5:
    ...
    def add_on_shutdown(self, method):
        self._shutdown_callbacks.append(method)

    def shutdown(self):
        print("Shutting down Gazebo World")
